...
class Maze:
    ...
    def distance_to_walls(self, x, y):
        '''
        Measure the distance of coordinates to the nearest walls at four directions.
...
class Particle:
    ...
    def try_move(self, speed, noisy=False):
        ...
        # Move across one grid horizontally
        ...
        # Move across grids both vertically and horizontally
...
class Robot(Particle):
    ...
    def weight_gaussian_kernel(self, x1, x2, std=10):
        ...
        return np.exp(-(np.abs(x1 - x2) ** 2) / (2 * std))
...
for j in range(0, num_iter):
    ...
    for particle in particles:
        ...
        particle.weight = bob.weight_gaussian_kernel(x1=readings_particle, x2=readings_robot, std=kernel_sigma)
    ...
    for i in range(num_particles):
        ...
        particles_new.append(
            Particle(x=particle.x, y=particle.y, maze=maze, heading=particle.heading, sensor_limit=sensor_limit,
                     noisy=True))

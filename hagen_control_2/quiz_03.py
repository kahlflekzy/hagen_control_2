...
class EKFLocalization():
    def __init__(self, dt, var_v, var_z, map_data):
        ...
        self.state, self.control = None, None
        self.f, self.f_j, self.v_j = None, None, None
    ...
    def measurement_model(x, m):
        ...
        for i in range(m.shape[0]):
            ...
            m_ = m[i, :]
            x1, y1, z1 = m_
            d = np.sqrt(((x_ - x1)**2) + ((y_ - y1)**2) + (z_ - z1)**2)
            z[i] = d
        return z  # Task 2
    ...
    def simulation_step(self, x, u):
        ...
        zt = zt_bar + self.sample_normal_distribution(self.var_z) # Task 3
    ...
    def ekf(self, muy, sigma, u, z, m):
        ...
        muy_bar = self.motion_model(muy, u)  # Task 4
        # predict covariance
        ...
        for i in range(self.num_beacons):
            # expected measurement for the ith measurement
            z_hat = expected_meas[i] # Task 5
